...
usage() {
    ...
    echo '      -a  Creates an archive of the home directory assocoated with the account(s).' >&2
    exit 1
}
...
for USERNAME in "${@}"; do
    ...
    if [[ "${ARCHIVE}" = 'true' ]]; then
        ...
        if [[ ! -d "${ARCHIVE_DIR}" ]]; then
            echo "Createing ${ARCHIVE_DIT} directory."
            mkdir -p ${ARCHIVE_DIR}
            if [[ "${?}" -ne 0 ]]; then
    ...
    if [[ "${DELETE_USER}" = 'true' ]]; then
        ...
        # Check to see if the userdel commanded.
        ...
        # We don't want to tell the user that an acoount was disabled when it hasn't been.
        if [[ "${?}" -ne 0 ]]; then
